...
Commands:
    ...
    db-reset            Delete & recreate the Postgres volume (loses all data!)
...
cmd_db_reset() {
    if [ "$ENVIRONMENT" = "local" ]; then
        VOLUME_NAME="docucast_postgres_local_data"
    else
        VOLUME_NAME="docucast_postgres_data"
    fi

    log_warning "This will DELETE the PostgreSQL data volume: $VOLUME_NAME"
    log_warning "ALL DATABASE DATA WILL BE PERMANENTLY LOST."
    log_warning "Use this to recover from a role/credential mismatch on a fresh deploy."
    ...
    if [ "$CONFIRM" = "yes" ]; then
        ...
        docker volume rm "$VOLUME_NAME" 2>/dev/null || log_warning "Volume not found or already removed"

        log_info "Restarting stack..."
        docker_compose up -d

        log_success "Postgres volume recreated. Run './docker-manage.sh migrate' once the stack is healthy."
    else
